...
import { type AzureContainerAppsTestApi } from '../../src/testApi';

let cachedTestApi: AzureContainerAppsTestApi | undefined;
 ...
 */
export async function getTestApi(): Promise<AzureContainerAppsTestApi> {
    if (!cachedTestApi) {
        ...
        cachedTestApi = apiProvider.getApi<AzureContainerAppsTestApi>('>=99.0.0');
 ...
 */
export function getCachedTestApi(): AzureContainerAppsTestApi {
    if (!cachedTestApi) {
