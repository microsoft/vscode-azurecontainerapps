...
import { parseLocalPlanMarkdown } from "../../utils/parseLocalPlanMarkdown";
import { LocalPlanViewController } from "../../webviews/LocalPlanViewController";
...
export async function openLocalPlanViewFromWorkspace(): Promise<void> {
    const files = await vscode.workspace.findFiles('**/local-dev.plan.md', '**/node_modules/**', 10);
    if (files.length === 0) {
