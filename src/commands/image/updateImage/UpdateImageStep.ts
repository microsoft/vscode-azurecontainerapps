...
import { nonNullProp, nonNullValueAndProp, randomUtils } from "@microsoft/vscode-azext-utils";
...
export class UpdateImageStep<T extends UpdateImageContext> extends RevisionDraftUpdateBaseStep<T> {
    ...
    public async execute(context: UpdateImageContext, progress: Progress<{ message?: string | undefined; increment?: number | undefined }>): Promise<void> {
        ...
        ext.outputChannel.appendLog(localize('updatedImage', 'Updated container app "{0}" with image "{1}" (draft).', nonNullValueAndProp(context.containerApp, 'name'), context.image));
    }
