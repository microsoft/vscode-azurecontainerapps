...
export class SecretDeleteConfirmStep extends AzureWizardPromptStep<ISecretContext> {
    public async prompt(context: ISecretContext): Promise<void> {
        await context.ui.showWarningMessage(
            localize('secretDeleteWarning', 'Are you sure you want to delete secret "{0}"? Deletion is blocked while the secret is still in use.', nonNullProp(context, 'secretName')),
            { modal: true },
