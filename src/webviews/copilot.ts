...
export function createPrimaryPromptToGetSingleQuickPickInput(picks: string[], relevantContext?: ViewPropertiesModel): string {
    return `The User is asking you to choose an item based on the following information:
        ...
        3. Use information from this context if available: ${relevantContext ? JSON.stringify(relevantContext) : "No context available"}
...
export function createPrimaryPromptToGetPickManyQuickPickInput(picks: string[], relevantContext?: ViewPropertiesModel): string {
...
export function createPrimaryPromptForInputBox(inputQuestion: string, relevantContext?: ViewPropertiesModel): string {
    return `The User is asking you to provide an input based on the following question:
        1. ${inputQuestion}
        2. Use information from this context if available: ${relevantContext ? JSON.stringify(relevantContext) : "No context available"}
        Respond with a string containing the input value. Do not respond in a conversational tone, only a single.`;
}

export function getAzureResourcesForACADeploy(): void {
    // get container apps
}
